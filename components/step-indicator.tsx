...
export default function StepIndicator({ steps }: Readonly<StepIndicatorProps>) {
  return (
    <div className="space-y-8 w-full space-x-4">
      ...
      <div className="space-y-6">
        {steps.map((step) => {
          const stepStatusClass = step.active || step.completed ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-600"

          return (
            <div key={step.number} className="flex items-start gap-4">
              <div
                className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm ${stepStatusClass}`}
              >
